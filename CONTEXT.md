...
1. **Tails:** для MS/RS проверить MAX+1, при нахождении — экспоненциальный поиск новой границы
2. **New prefixes:** перепроверить registry.empty (могли появиться новые префиксы)
3. **Revalidate (--full):** запросить каждый код, сравнить поля
